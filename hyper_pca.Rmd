...
#### combine mammal and bird trait data
...
pc_mi_d1 <- pc_mi_d1 %>% 
  ...
  # calculate mean per species for Comp.1
  dplyr::mutate(Comp.1_mean = apply(dplyr::select(., contains("Comp.1")), 1, mean)) %>% 
  # calculate variance for Comp.1
  dplyr::mutate(Comp.1_B = apply(dplyr::select(., contains("Comp.1")), 1, var)) %>% 
  # calculate Rubin's T for Comp.1
  dplyr::mutate(Comp.1_T = Comp.1_B + Comp.1_B/25) %>% 
  # standardize Rubin's T for Comp.1
  dplyr::mutate(Comp.1_T_std = rescale3(-Comp.1_T)) %>% 
  # repeat for other principal components
  dplyr::mutate(Comp.2_mean = apply(dplyr::select(., contains("Comp.2")), 1, mean)) %>% 
  dplyr::mutate(Comp.2_B = apply(dplyr::select(., contains("Comp.2")), 1, var)) %>% 
  dplyr::mutate(Comp.2_T = Comp.2_B + Comp.2_B/25) %>% 
  dplyr::mutate(Comp.2_T_std = rescale3(-Comp.2_T)) %>% 
  dplyr::mutate(Comp.3_mean = apply(dplyr::select(., contains("Comp.3")), 1, mean)) %>% 
  dplyr::mutate(Comp.3_B = apply(dplyr::select(., contains("Comp.3")), 1, var)) %>% 
  dplyr::mutate(Comp.3_T = Comp.3_B + Comp.3_B/25) %>% 
  dplyr::mutate(Comp.3_T_std = rescale3(-Comp.3_T)) %>% 
  dplyr::mutate(Comp.4_mean = apply(dplyr::select(., contains("Comp.4")), 1, mean)) %>% 
  dplyr::mutate(Comp.4_B = apply(dplyr::select(., contains("Comp.4")), 1, var)) %>% 
  dplyr::mutate(Comp.4_T = Comp.4_B + Comp.4_B/25) %>% 
  dplyr::mutate(Comp.4_T_std = rescale3(-Comp.4_T)) %>% 
  dplyr::mutate(Comp.5_mean = apply(dplyr::select(., contains("Comp.5")), 1, mean)) %>% 
  dplyr::mutate(Comp.5_B = apply(dplyr::select(., contains("Comp.5")), 1, var)) %>% 
  dplyr::mutate(Comp.5_T = Comp.5_B + Comp.5_B/25) %>% 
  dplyr::mutate(Comp.5_T_std = rescale3(-Comp.5_T))
...
pcload_mi_d1 <- pcload_mi_d1 %>% 
  ...
  # calculate variance for Comp.1
  dplyr::mutate(Comp.1_B = apply(dplyr::select(., contains("Comp.1")), 1, var)) %>% 
  # calcualte Rubin's T for Comp.1
  dplyr::mutate(Comp.1_T = Comp.1_B + Comp.1_B/25) %>% 
  # standardize Rubin's T for Comp.1
  dplyr::mutate(Comp.1_T_std = rescale3(-Comp.1_T)) %>% 
  # repeat for other principal components
...
null_1_samp <- replicate(npermute, dplyr::sample_n(tr_ex, nrow(prob_ex_m100[[1]])), simplify = FALSE)

null_1 <- lapply(null_1_samp, function(x) {
  a <- dplyr::filter(tr_ex, !binomial %in% x$binomial)
  b <- a %>% 
    dplyr::select(-ex) %>% 
    tibble::column_to_rownames(var = "binomial")})

#null_1_hyper <- lapply(null_1, hypervolume_svm)

# 47 secs per hypervolume (~13 hours for 999)

# save: null_1_hyper
#saveRDS(null_1_hyper, "data/hv_null_m100_hyper.rds")

# load: null_1_hyper
null_1_hyper <- readRDS("data/hv_null_1_hyper.rds")

null_1_vol <- sapply(null_1_hyper[1:npermute], function(x) return(x@Volume)) %>% 
  as.data.frame()

colnames(null_1_vol) <- c("vol")

# compare predicted to null

# kololmogorov-smirnov test
ks.test(exp_vol_m100$vol, null_1_vol$vol, alternative = "greater") # greater - distributions for which x is stochastically smaller than y (the CDF of x lies above and hence to the left of that for y)

# effect size
obs_hyper@Volume - mean(exp_vol_m100$vol)

obs_hyper@Volume - mean(null_1_vol$vol)

diff <- data.frame(obs = rep(obs_hyper@Volume, npermute), exp = exp_vol_m100$vol, null = null_1_vol$vol) %>% 
...
#### plot difference between projected and randomized

```{r}
group <- c(rep("exp", npermute), rep("null", npermute))
dat <- data.frame(vol = c(exp_vol_m100$vol, null_1_vol$vol), group = group)
...
null_1_td <- lapply(null_1_samp, function(x) {
  a <- filter(tr_mi, !binomial %in% x$binomial)
  b <- a %>% 
    column_to_rownames(var = "binomial")})

tr_null_td <- bind_rows(null_1_td, .id = "run")
